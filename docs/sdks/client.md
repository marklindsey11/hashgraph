...
| `Client.<network>.setNetworkName(<networkName>)`        | NetworkName             | <p>Set the network name to a particular value. Useful when constructing a network which is a subset of an existing known network [Deprecated <code></code> <br><code></code>Use <code>Client.[set|get]LedgerId()]</code></p> |
