...
| `[Deprecated] Client.<network>.setNetwork(<nodes>`      | Map\<String, AccountId> | <p>Replace all nodes in this Client with a new set of nodes (e.g. for an Address Book update)<br><br>Use <code>Client.[set|get]LedgerId()</code></p>            |
| `Client.<network>.setNetworkName(<networkName>)`        | NetworkName             | Set the network name to a particular value. Useful when constructing a network which is a subset of an existing known network                                   |
